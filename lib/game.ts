...
export type Room = { id: string; host: string; players: Player[]; messages: Message[]; direct?: boolean; phase: 'waiting'|'playing'|'finished'; turn: string; last: string; used: string[]; deadline: number; seconds: number; vocabulary?: 'basic'|'extended'; turnCount: number; custom: string[]; pending: {word:string; player:string} | null; created: number };
export function message(r:Room, name:string, text:string, system=false) {r.messages.push({id:crypto.randomUUID(),name,text,time:Date.now(),system,senderId:system?undefined:r.players.find(p=>p.name===name)?.id.slice(0,8)}); r.messages = r.messages.slice(-150);}

...
assert.equal(quizText.normalizeAnswer(' 세종 대왕! '),quizText.normalizeAnswer('세종대왕'));assert.equal(quizText.choseongHint('세종 대왕'),'ㅅㅈ ㄷㅇ');
...
editQuiz(state=>state.nextAt=Date.now()-1);await call(quiz,a,'get',{room:quizRoom});editQuiz(state=>state.openedAt=Date.now()-6000);await call(quiz,a,'get',{room:quizRoom});assert(rawQuiz().messages.some(message=>message.text.includes('힌트 ·')));
...
editQuiz(state=>state.nextAt=Date.now()-1);await call(quiz,a,'get',{room:quizRoom});editQuiz(state=>state.openedAt=Date.now()-16000);await call(quiz,a,'get',{room:quizRoom});assert.equal(rawQuiz().quiz.stage,'result');assert(rawQuiz().messages.some(message=>message.text.includes('추가 힌트')));assert(rawQuiz().messages.some(message=>message.text.includes('시간이 지났습니다.')));
...
console.log('PASS: quiz single/multiplayer, four difficulties, hints, reveal, normalization, cooldown, CAS first answer, scoring, history, fingerprint dedup, cached fallback and OpenTDB session token');
